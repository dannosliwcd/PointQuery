...
TEST_CASE("search in empty finder")
{
	...
	finder = PointFinder::Make(PointFinder::Method::Heap, records);
	REQUIRE(finder->FindNearest(0, 0, 1).empty());
}
...
TEST_CASE("search for exact points")
{
	auto test = [](PointFinder::Method method)
	{
		...
		for (const auto& record : records)
		{
			...
			REQUIRE(record == nearest.front().second);
		}
	};
...
TEST_CASE("Search for a trivially close point")
{
	auto test = [](PointFinder::Method method)
	{
		...
		REQUIRE(nearest.size() == 1);
		REQUIRE(records[0] == nearest.front().second);
	};
...
TEST_CASE("bottom of valley")
{
	auto test = [](PointFinder::Method method)
	{
		...
		REQUIRE(nearest.size() == 1);
		REQUIRE(records[4] == nearest.front().second);
	};
...
TEST_CASE("surrounding a central point")
{
	auto test = [](PointFinder::Method method)
	{
		...
		SECTION("in between")
		{
			...
			REQUIRE(nearest.size() == 1);
			REQUIRE(records[8] == nearest.front().second);
		...
		SECTION("2 nearest")
		{
			...
			CHECK(records[4] == nearest[0].second);
			REQUIRE(records[0] == nearest[1].second);
		}
	};
